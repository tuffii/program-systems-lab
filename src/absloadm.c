...
/*
***** Convert EBCDIC byte to ASCII, return '.' for non-printable
*/
char ebcdic_to_print(unsigned char ebcdic_byte)
{
  unsigned char ascii = EBCDIC_TO_ASCII[ebcdic_byte];
...
int wind(void)
{
  ...
  for (j1 = 0; j1 < 8; j1++)
  {
    ...
    for (j = 0; j < 16; j++)
    {
      waddch(wred, ebcdic_to_print(OBLZ[line_kk + j]));
    }
